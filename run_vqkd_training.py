...
def get_args():
    ...
    parser.add_argument('--load_weights', default=None, help='loading weights for vqkd')
...
def get_model(args, **kwargs):
    ...
    model = create_model(
        args.model,
        pretrained=False, #Changed from False to True - Load my weights on the model
        as_tokenzer=False,
    ...
    if args.load_weights:  #added this to load weights
         checkpoint=torch.load(args.load_weights,map_location='cpu')
         model.load_state_dict(checkpoint, strict=False)
         #print(model.encoder.weight)
     #model.load_state_dict(torch.load('home/sfatima/ImageNet/Weights/checkpoint.pth'))
    return model
